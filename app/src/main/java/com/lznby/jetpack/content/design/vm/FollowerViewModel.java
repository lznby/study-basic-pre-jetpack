...
public class FollowerViewModel extends BaseActivityViewModel<FollowerActivity, List<UserFollowerInfoEntity>> {
    ...
    private void getFollowers(String userId, String who, String type) {
        ...
        if (who.equals(Configure.RouterType.MINE) && type.equals(FollowerRouterEntity.FOLLOWER)) {
            ...
            if (!"".equals(cacheString)) {
                entities = gson.fromJson(cacheString, new TypeToken<List<UserFollowerInfoEntity>>() {
                }.getType());
            }
            if (entities != null) {
    ...
    private void updateUi(BaseEntity entity, boolean isFollow, int position) {
        if (entity.getCode() == Configure.ResponseCode.SUCCESS && Objects.requireNonNull(getLiveData().getValue()).size() > position) {
            if (isFollow) {
                getLiveData().getValue().get(position).setIsFollow(1);
            } else {
                getLiveData().getValue().get(position).setIsFollow(0);
            }
     ...
     * @param item
     */
    public void follow(UserFollowerInfoEntity item, int position) {
        addDisposable(
     ...
     * @param item
     */
    public void unFollower(UserFollowerInfoEntity item, int position) {
        addDisposable(
